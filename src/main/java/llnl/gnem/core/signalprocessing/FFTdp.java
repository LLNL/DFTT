...
//  workspace for the radix-R extenders rvfftRX and irvfftRX
    ...
    public FFTdp( int _m, int _R )
    {
        ...
        if( R > 1 ) {
            xr = new Vector<double[]>( R );
            for ( int r = 0; r < R; r++ ) {
                xr.addElement( new double[M] );
            }
        }
        else
            xr = null;
...
//  Double precision version by D. Harris 12/24/03

    public static void rvfft( double[] x, int _m )
    {

        int n = 1;
        for ( int i = 0; i < _m; i++ ) n *= 2;
...
//-------L shaped butterflies------------------------------------
    ...
    private static void stage( int N, int N2, int N4,
                               double[] X,
                               int OFF2, int OFF3, int OFF4 )
    {
        ...
        if( N4 > 1 ) {
            ...
            do {
                for ( I2 = IS + N8; I2 < N; I2 += ID ) {
                    T1 = ( ( X[OFF3 + I2] + X[OFF4 + I2] )
                            * .7071067811865475 );
                    T2 = ( ( X[OFF3 + I2] - X[OFF4 + I2] )
                            * .7071067811865475 );
                ...
                for ( J = 2; J <= N8; J++ ) {
                    ...
                    do {
                        for ( I = IS + J; I <= N; I += ID ) {
                            I1 = I - 1;
                            I2 = I1 + JN;
                            T1 = ( X[OFF3 + I1] * CW1R + X[OFF3 + I2] * SW1I );
                            T2 = ( X[OFF3 + I2] * CW1R - X[OFF3 + I1] * SW1I );
                            T3 = ( X[OFF4 + I1] * CC3 + X[OFF4 + I2] * SS3 );
                            T4 = ( X[OFF4 + I2] * CC3 - X[OFF4 + I1] * SS3 );
                            T5 = T1 + T3;
                            T6 = T2 + T4;
                            T3 = T1 - T3;
                            T4 = T2 - T4;
...
//  Double precision version by D. Harris    12/24/03
    ...
    public static void irvfft( double[] x, int _m )
    {
...
//-------Divide by N---------------------------------------------
    ...
    private static void istage( int N, int N2, int N4,
                                double[] X,
                                int OFF2, int OFF3, int OFF4 )
    {

/*C===============================================================C
  C     function istaged - the work-horse of the IFFT             C
  C===============================================================C */
                ...
                for ( J = 2; J <= N8; J++ ) {
                    ...
                    do {
                        ...
                        for ( I = IS + J; I <= N; I += ID ) {
                            ...
                            T4 = T2 - T3;
                            T2 = T2 + T3;
                            X[OFF3 + I1] = ( T5 * CW1R + T4 * SW1I );
                            X[OFF3 + I2] = ( -T4 * CW1R + T5 * SW1I );
                            X[OFF4 + I1] = ( T1 * CC3 - T2 * SS3 );
                            X[OFF4 + I2] = ( T2 * CC3 + T1 * SS3 );
                        }
                        IS = 2 * ID - N2;
                        ID = 4 * ID;
                    } while( IS < N );
    ...
    public void rvfftRX( double[] x )
    {
        ...
        for ( int i = 0; i < M; i++ ) {
            ...
            for ( int p = 1; p < pmax; p++ ) {
                ...
                pr = 0;
                for ( int r = 0; r < R; r++ ) {
                    Yr += WRr[pr] * tmpr[r] - WRi[pr] * tmpi[r];
                    Yi += WRr[pr] * tmpi[r] + WRi[pr] * tmpr[r];
                    pr += p;
                }
                x[k] = Yr;
                x[N - k] = Yi;
    ...
    public void irvfftRX( double[] x )
    {
        ...
        for ( int r = 1; r < R; r++ ) {
            ...
            for ( int i = 0; i <= M / 2; i++ ) {
                ...
                xrptr[i] = WNr * Yr - WNi * Yi;
                if( i != 0 && i != M / 2 ) xrptr[M - i] = WNr * Yi + WNi * Yr;
